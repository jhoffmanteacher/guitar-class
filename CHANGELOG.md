...
- **The "Open the Song Journey page" button on a class activity is at the
  ...
  song page, after you have done the steps before it. Every class activity
  works this way, including new ones.
...
- **Your metronome keeps running when you tick a step done or leave the
  page.** If you had the metronome going while you worked, ticking a step off,
  marking a card complete, or closing In-Class Activities switched it off
  along with the card's own sound. It keeps going now, and stops when you stop
  it. Anything the card itself was playing still stops, and the metronome
  still gets out of the way when the Listening Coach or the tuner needs the
  mic.
...
### Changed
- **"the cure" — Intro and Verse on the Low E.** The activity that was Verse
  and Chorus Roots now opens with the song's intro — the guitar on its own, A
  then C, twice — teaches the verse, and stops there. The chorus moved to the
  next day's card, so one class is one new section instead of two. The whole
  card is on the low E string: A at fret 5, C at fret 8, F at fret 1, with the
  hand travelling between them. The next day's card is where the same line
  moves onto two strings and stops travelling.

### Added
- **New activity: "the cure" — Intro, Verse and Chorus Roots.** The next day's
  card. The intro and the verse again from the top, this time on the low E and
  A strings so nothing sits past fret 5, then the chorus — the last two roots,
...
- **Inline TAB is wider and shorter.** The fret numbers are a little smaller
...
- **Module 2's "Take It to a Song" now links straight to the Song Journey** —
  six buttons, one per song, instead of its own two steps.
- **Module 2 drops its Checkpoint, Wrap-Up, Practice Routine and Ear Spark
  prompts** from the lesson ladder — reflection and bonus content the module
  doesn't need alongside the day's activities.
- **Three "Seven Nation Army" ear/preview steps in Module 2 Set 1 are gone**
  — the in-class Seven Nation Army activity now teaches them.
- Nothing about the skills checklist changed — every skill in Module 2 still
  checks off exactly as it did.

This is a pilot on one module (Today-first work order, Phase 3) — the rest
of the course is unchanged for now.
...
- **Station B and Station C are now a single list.** Every set used to split
  its work into two panels you had to switch between — one for the videos,
  one for the drills. They're one continuous ladder now: the watching steps
  first, then a line that reads "Now practice it — …", then the drills, with
  the step numbers running straight through (Step 9 of 17, not Step 3 of 8
  in a second panel). Nothing was added or removed — every step, every
  video, every challenge is exactly where it was, and everything you'd
  already checked off is still checked off.
- **The menu is simpler.** "This set" now has two buttons instead of three:
  **The lesson** and **My skills checklist**.
- **The two reflection cards have clearer names.** The one in the middle of
  the set is now **Checkpoint** (*Punto de control*); the one at the end is
  **Wrap-Up** (*Cierre*). They used to both be called "Station Wrap-Up",
