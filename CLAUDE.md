...
## i18n — hand-written Spanish for the app shell
The Español button drives **two layers at once**: `i18n.js` (hand-written
Spanish for the app *shell* — header, nav rail, tool popups, skills
checklist, buttons/footer) plus the existing Google Translate widget
(everything else — module/lesson content, still machine-translated, an
acceptable stopgap for hundreds of lesson steps but not for words a student
reads every session).
...
- **How to wire a new string** — full mechanics are documented at the top of
  ...
  - `translate="no"` is added **automatically** by `applyI18n()` to anything
    it touches — never add it by hand. This is what stops Google Translate
    from re-translating our already-Spanish text when both layers are on at
    once.
- `translations-review.md` (repo root) holds the full EN → ES table in one
  place — hand it to a bilingual colleague or student aide for a human
  spot-check; keep it in sync with `i18n.js` (it's generated by eye from the
  table, not auto-built, so update both in the same edit).
- `i18n.js` loads on **every page that uses these strings**: `index.html`
  (the full shell) and every `tabs/*.html` Song Journey page (just the
  shared floating Tuner/Timer/Metronome markup — the rest of a Journey page
  is still Google-Translate-only, matching the site's original scope there).
  It must load **before** `app.js`, `fab-tools.js`, and `tuner.js` on every
  page, since all three call `t()`/reference `data-i18n` at parse or event
  time.
