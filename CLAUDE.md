...
- `translations-review.md` (repo root) holds the full EN → ES table in one
  place; keep it in sync with `i18n.js` (it's generated by eye from the
  table, not auto-built, so update both in the same edit). **Review policy
  (2026-07-23): no bilingual human is available, so the QA gate is an
  independent AI proofread sweep** — fresh agents with no authorship context
  audit new Spanish against the English + glossary and flag genuine errors
  only (see the sheet's "Review policy" section). Run one after any large
  batch of new Spanish.
