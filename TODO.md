...
`module-3.js`…`module-8.js` (8 warm-up instances, Module 4 ×3): dropped "and at the
1st–3rd fret, listening for buzz" from the step text, retitled the section "Warm-up —
tuning check (Module 1)", removed "and every note clean" from the Win line, and cut the
"clean, buzz-free notes (Module 2)" reference from each hint. The warm-up is now a pure
tuning check. (The "Clean E5"/"Clean Em" challenge lines that legitimately use "buzz-free"
were left intact.)
