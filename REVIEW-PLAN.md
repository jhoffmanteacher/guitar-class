...
#### K-3 — Auto-retry failed progress saves (CONFIRMED)
...
#### K-4 — De-duplicate Firebase bootstrap + SDK version (8 copies)
...
#### K-6 — `toggleSkill` should select rows by data attribute, not regex-parse `onclick` strings
...
#### K-7 — Fix or remove the dead `#games` deep-link helper
...
#### K-8 — Update stale "8 modules" references (comments + docs)
...
#### C-1 — Unify "Module" vs "Unit" terminology (CONFIRMED)
...
#### C-2 — Module 8 fingerstyle links promise an arrangement the journey pages don't have (CONFIRMED)
...
#### C-3 — Journey footers claim "60 BPM throughout" where the body ramps higher
...
#### C-4 — "Core" tag is reused for module-required songs that aren't thread songs
...
#### C-5 — Module 5 Set 3: give Bm a dedicated clean-fret drill
...
#### C-6 — Sweet Child journey Layer-1 labeling inconsistency
...
#### C-8 — Two Station B pairs use the same instructor twice (optional)
...
#### D-1 — Bring the journey pages onto the app's theme, including dark mode (CONFIRMED)
...
#### D-2 — Fix WCAG AA contrast failures on journey helper text (CONFIRMED)
...
#### D-3 — Add real headings to generated practice content (CONFIRMED)
...
#### D-4 — Journey "Rate this layer" buttons: group semantics + touch size
...
#### D-5 — Add the Spanish-translate affordance to journey pages
...
#### D-6 — Welcome onboarding dialog focus management
...
#### D-7 — Tuner string-selector touch targets
...
#### D-8 — Locked set pills drop below readable contrast
...
#### I-2 — Persist practice-tempo (BPM) choices across sessions
...
#### I-5 — Global practice streak + "Done today" on the Daily 5
...
#### I-6 — "Keep practicing" review list from 'still working on it' marks
...
#### I-7 — Teacher "trouble spots" view
...
#### I-8 — Student "My progress" all-modules overview
...
#### I-9 — Persist a compact Listening Coach result per drill
...
- **I-10 — PR-BPM history:** append `{value, date}` (capped ~8) for `isPR` response slots instead of overwriting (`teacher.js:202`, `app.js:327`); render a tiny trend. *Done when successive PR entries preserve and display earlier ones.*
- **I-11 — Metronome downbeat accent + count-in:** time-signature selector (2/4, 3/4, 4/4) accenting beat 1, optional one-bar count-in (`app.js:2752, 2756`; `index.html:189`). *Done when beat 1 is audibly accented for a chosen meter.*
- **I-12 — Offline banner:** on window `offline`, show a dismissable "You're offline — practice pages still work; videos and saving resume when you reconnect" banner reusing the safety-net styling (`app.js:7–37`; `sw.js:78`). *Done when going offline shows it and reconnecting clears it.*
- **I-13 — "What can I play?" chord search:** index songs by chord set in `buildSearchIndex` (`app.js:3216, 3256`) so "G C D" returns songs playable with those shapes. *Done when a chord-set query returns matching songs.*
