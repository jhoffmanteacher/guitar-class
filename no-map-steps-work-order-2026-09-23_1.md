...
### 5b. Remove the duplicated band player (one thing per step)

Two activities put the same `snippet` block on both the tab step and the band step, so the card shows the band player twice in a row. The tab step plays with the tab; the band step plays with the band. Delete the `snippet` object from the **tab** step only:

- `ca-19` step "Practice — The whole verse" — delete its `snippet` (the-cure, fromBar 5, bars 8). Keep the identical `snippet` on "Practice — With the band".
- `ca-18` step "Practice — The song in order" — delete its `snippet` (the-cure, fromBar 1, bars 28). Keep the identical `snippet` on "Practice — With the band".

Leave `ca-10` steps 4 and 5 alone — their windows differ (4 bars vs 8 bars), not a duplicate. Leave every other `snippet` alone.
...
- `ca-19` and `ca-18`: the orange band player appears once, on the last step only; the step before it shows only the tab.
